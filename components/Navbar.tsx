    ...
    <nav className="bg-white shadow-sm border-b sticky top-0 z-50">
      <div className="max-w-4xl mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <Link href="/" className="flex items-center gap-2">
          {salonLogo ? (
            <img src={salonLogo} alt="logo" className="w-8 h-8 rounded-full object-cover" />
          ) : (
            <span className="text-2xl">💅</span>
          )}
          <span className="font-bold text-gray-900">{salonName}</span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          {user ? (
                  ...
                  <div className="absolute left-0 mt-2 w-52 bg-white rounded-xl shadow-lg border z-50 overflow-hidden">
                    <div className="px-4 py-3 bg-pink-50 border-b">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full mt-1 inline-block ${
                        user.role === "admin" ? "bg-purple-100 text-purple-600" : "bg-pink-100 text-pink-600"
                      }`}>
                        {user.role === "admin" ? "ادمین" : "مشتری"}
                      </span>
                    </div>
                    <Link href="/profile" className="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 hover:bg-pink-50" onClick={() => setMenuOpen(false)}>
                      <span>👤</span> پروفایل من
                    </Link>
                    {user.role === "admin" && (
                      <Link href="/admin/dashboard" className="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 hover:bg-purple-50" onClick={() => setMenuOpen(false)}>
            ...
            <Link href="/login">
              <Button size="sm" variant="outline">ورود / ثبت‌نام</Button>
            </Link>
          )}
        </div>
        {/* همبرگر فقط md به بالا — موبایل bottom nav داره */}
      </div>
    </nav>
  )
}